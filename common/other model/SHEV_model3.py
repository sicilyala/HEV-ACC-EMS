...
class SHEV_model:
    def __init__(self):
        ...
        self.nominal_life = pow(20/self._severity_funct(0.35, 2.5, 25), 1/0.57)
    ...
    def _func_init(self):
        ...
        data_dir = "D:/SEU2/Program1/MADDPG-program/common/"
        # Engine
    ...
    @staticmethod
    def _severity_funct(soc, I_c, T_batt):
        if soc >= 0.45:
            B = 2694.5*soc+6022.2
        else:
            B = 2896.6*soc+7411.2
        E_a = 31500  # activation energy, J/mol
        R_g = 8.31  # Universal gas constant
        T_kelvin = 273.15+T_batt
        sigma = B*math.exp((152.5*I_c-E_a)/(R_g*T_kelvin))
        return sigma
    
    def run_EGS(self, car_spd, car_acc, eng_power):
        T_axle, W_axle = self._T_W_axle(car_spd, car_acc)
        # P_axle = T_axle * W_axle  # 传动轴power
        if eng_power > 62000:
            eng_power = 62000
        ...
        if (eng_power < 500) or T_axle < 0:
            T_eng = 0
            W_eng = 0
        Eng_fuel_mdot = self.Eng_fuel_func(W_eng, T_eng)  # list
        Eng_fuel_mdot = Eng_fuel_mdot[0]
        # ISG
        T_ISG = T_eng
        W_ISG = W_eng
        # ISG s.t.
        ...
        P_batt_by_EGS = P_ISG+P_mot     # P_batt_by_EGS = P_mot - P_ISG        # TODO ??
        np.save(self.power_data, [P_batt_by_EGS])
        fuel_cost = Eng_fuel_mdot
        out_info = {'engine speed': W_eng, 'engine torque': T_eng, 'engine power': P_eng,
                    'motor speed': W_mot, 'motor torque': T_mot, 'motor power': P_mot, 'Mot_eff': Mot_eff,
                    'ISG speed': W_ISG, 'ISG torque': T_ISG, 'ISG power': P_ISG, 'Gen_eff': Gen_eff,
                    'fuel cost': Eng_fuel_mdot}
        # 'EGS action': eng_power}
        return W_eng, T_eng, fuel_cost, out_info
    ...
    def run_battery(self, P_batt, SOC, Tep_c, Tep_s):
        ...
        if I_batt > self.I_max:
            I_batt = self.I_max
        self.accumulated_Ah += abs(I_batt)*1/3600  # 1s
        # Battery temperature
        Tep_e = 25  # 环境的温度,摄氏度
        ...
        if Tep_s > 55:
            Tep_s = 55
        Tep_a = (Tep_c+Tep_s)/2
        I_c = abs(I_batt)/self.Q_batt  # TODO: 取负号的情况？
        # New SoC
        SOC_new = -I_batt/self.Q_batt/3600 + SOC
        SOC_new = (np.conjugate(SOC_new)+SOC_new)/2  # conjugate 共轭
        if SOC_new > 1:
            SOC_new = 1
        estimated_life = pow(20/self._severity_funct(SOC_new, I_c, Tep_a), 1/0.57)
        severity_factor = self.nominal_life/estimated_life
        severity_cost = severity_factor*abs(I_batt)  # TODO
        x = np.load(self.power_data)
        P_batt_by_EGS = x[0]
        power_difference = abs(P_batt-P_batt_by_EGS)  # TODO
        out_info = {'SoC': SOC_new, 'accumulated Ah': self.accumulated_Ah, 'battery power': P_batt,
                    'battery temperature': Tep_a, 'current': I_batt, 'current rate': I_c,
                    'severity_factor': severity_factor, 'power_difference': power_difference,
                    'P_batt_by_EGS': P_batt_by_EGS, 'resistance': resistance, 'battery OCV': V_batt,
                    'estimated_life': estimated_life}
        return SOC_new, Tep_c, Tep_s, Tep_a, severity_cost, power_difference, P_batt_by_EGS, I_batt, out_info