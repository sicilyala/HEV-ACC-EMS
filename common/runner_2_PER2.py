...
from brain_PER import Brain
from Priority_Replay import Memory_PER
...
class Runner:
    def __init__(self, args, env):
        ...
        self.memories = [Memory_PER(args), Memory_PER(args)]
    ...
    def run(self):
        ...
        for episode in tqdm(range(self.episode_num)):
            ...
            while episode_step < self.episode_step:
                ...
                # learn
                if self.memories[0].current_size >= 10*self.args.batch_size:
                ...
                #     s0, a0, r0, s_0, s1, a1, r1, s_1 = self.seperate_transitions(transitions)
                    for agent_id, agent in enumerate(self.agents):
                        # unshared samples
                        tree_index, transitions, ISWeights = self.memories[agent_id].sample(self.args.batch_size)
                        s0, a0, r0, s_0, s1, a1, r1, s_1 = self.seperate_transitions(transitions)
                        other_agents = self.agents.copy()       # list
                        other_agents.remove(agent)
                        td_error_up = agent.learn(s0, a0, r0, s_0, s1, a1, r1, s_1, other_agents, ISWeights)  # train
                        self.memories[agent_id].batch_update(tree_index, td_error_up)
                        # loss data
        ...
        #  print information
        print('buffer counter:', self.memories[0].counter)
        print('buffer current size:', self.memories[0].current_size)
        print('replay ratio: %.3f'%(self.memories[0].counter/self.memories[0].current_size)+'\n')
        print('done:', DONE)
        # find maximal
