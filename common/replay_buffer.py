...
class Buffer:
    def __init__(self, args):
        ...
        for i in range(self.args.n_agents):
            self.buffer['o_%d' % i] = np.empty([self.size, self.args.obs_shape[i]])         # state
            self.buffer['u_%d' % i] = np.empty([self.size, self.args.action_shape[i]])      # action
            self.buffer['r_%d' % i] = np.empty([self.size])                                 # reward
            self.buffer['o_next_%d' % i] = np.empty([self.size, self.args.obs_shape[i]])    # next state
        # thread lock   线程锁
